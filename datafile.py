...
def foo(ticker):
    ...
    col2.metric('1 Week Forecast Price: ',str(h))
    col2.metric('Williams Percent Range: ', str(customwr))
    col3.metric('Money Flow Index: ',str(custommfi))
    col1.metric('Current Price',round(live,2))
    col1.metric('Signal',recc,delta_color="green")
   
    col2.write('1 Year Price Target: ' + str(other['1y Target Est']))
    ...
    col1.write('Signal Points of Reason -')
    col1.write('W%R: '+str(customheight))
    col1.write('MFI State: '+str(customstate))
