...
def foo(ticker):
    ...
    with st.sidebar:
        st.metric('Signal',str(recc),delta_color="green")
        st.metric('Current Price',round(live,2),delta=str(pastprice)+'(24hr)')
        st.metric('1 Week Forecast Price: ',str(h),delta=str(difference)+'(7 Days)')
        st.write('---')
